...
Version: 28.4 (Strong Density Constraint Enforcement)
"""
...
# ================================================================
# 4. MULTI-TASK TRUE PINN MODEL (with strong density constraint)
# ================================================================
...
class MultiTaskTruePINN(nn.Module):
    ...
    def compute_loss(self, X_scaled, X_raw, y_true, epoch=0, max_epochs=4000,
        ...
        # --- Strong density penalty using L1 (absolute error) ---
...
def generate_full_pdf_report(api, mcc, pvpp, mgst, binder, pressure, speed, granule,
    ...
    pdf.cell(0, 6, "Generated by: Hybrid AI Framework v28.4", ln=True, align="C")
...
# ================================================================
# 8. TRAIN MODEL (updated w_density)
# ================================================================
...
# ================================================================
# 9. PREDICTION & PLOTS (updated to enforce density)
# ================================================================
...
st.set_page_config(page_title="PINN Framework v28.4", page_icon="🧬", layout="wide")
clamp_session_state()
...
with st.sidebar:
    ...
    **Multi-Task PINN:**
    ...
    - **Density penalty weight:** {W_DENSITY:.1f} (very strong)
    - **Density clipping:** ✅ Enabled
    - **All constraints unified** ✅
    """)
    st.info("🔬 **v28.4** — Strong Density Constraint Enforcement")
...
with col_right:
    ...
    if predict_btn:
        if abs(total - 100) > 0.1:
            ...
            kpi_cols[0].metric("Density (clipped)", f"{density:.3f}", delta=f"≤ {DENSITY_MAX:.2f} ✅" if density <= DENSITY_MAX else f"> {DENSITY_MAX:.2f} ❌")
            ...
            if tensile >= TENSILE_MIN and efrf < EFRF_MAX and density <= DENSITY_MAX:
                ...
                st.warning(f"⚠️ Density was clipped from higher value to {DENSITY_MAX:.2f} (physics constraint enforced)")
...
st.markdown("---")
st.caption(f"🔬 **Multi-Task True PINN — v28.4 (Strong Density Constraint Enforcement)**")
