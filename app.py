...
Version: 29.26 (Cloud-Optimized - Fast Load & Lightweight)
"""
...
# ================================================================
# 0. PARAMETERS (مُخفّفة للنشر السحابي)
# ================================================================
...
# --- إعدادات خفيفة جداً للنشر (Cloud-Optimized) ---
N_SAMPLES = 1500            # قلتُها من 12000 إلى 1500
ADAM_EPOCHS = 80            # قلتُها من 1000 إلى 80
MONOTONICITY_FREQUENCY = 10

# --- Parameters NSGA-II (خفيفة) ---
NSGA_POP_SIZE = 30
NSGA_GENERATIONS = 20
...
# ================================================================
# 3. PINN MODEL (خفيف جداً: 128 عصبوناً فقط)
# ================================================================
...
class MultiTaskTruePINN(nn.Module):
    def __init__(self, input_dim=13, output_dim=5):
        ...
        # تم التصغير إلى 128 عصبوناً بدلاً من 384
...
class NSGAII:
    ...
    def _simulated_binary_crossover(self, p1, p2):
        if np.random.random() > P_C: return p1.copy(), p2.copy()
...
# ================================================================
# 6. تحميل أو تدريب النموذج (مع التخزين المؤقت على السحابة)
# ================================================================

@st.cache_resource
def load_pinn_model():
    checkpoint_path = '/tmp/pinn_model_light.pt'  # مسار مؤقت على السحابة
    
    # إذا كان الملف موجوداً، اقرأه مباشرة (سرعة فائقة)
    if os.path.exists(checkpoint_path):
        st.caption("📂 Loading pre-trained model from cache...")
    ...
    # إذا لم يكن موجوداً، درّب نموذجاً خفيفاً واحفظه
    st.caption("🔄 Training lightweight model for cloud (first run only)...")
    ...
    loss_history = {'train': train_losses, 'val': val_losses, 'val_early': []}
    
    # حفظ النموذج في ذاكرة التخزين المؤقت
    torch.save({
    ...
    st.caption("✅ Model trained and cached successfully!")
    return model, scaler, y_scaler, feature_names, df, loss_history

# ================================================================
# 7. دوال التنبؤ والرسوم البيانية
# ================================================================
...
def plot_training_curves(loss_history):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=epochs, y=loss_history['train'], mode='lines', name='Training Loss', line=dict(color='blue')))
    fig.add_trace(go.Scatter(x=epochs[:len(loss_history['val'])], y=loss_history['val'], mode='lines', name='Validation Loss', line=dict(color='orange', dash='dash')))
    fig.update_layout(title='Training Curves (Lightweight)', xaxis_title='Epoch', yaxis_title='Loss', yaxis_type='log', height=400)
    return fig
...
st.set_page_config(page_title="PINN Cloud v29.26", page_icon="🧬", layout="wide")

st.markdown("""
<div style="background: linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%);
            padding: 1.5rem; border-radius: 1rem; margin-bottom: 1.5rem; text-align: center;">
    <h1 style="color: #ffffff; font-size: 2rem; margin: 0;">🧬 Hybrid AI Framework v29.26</h1>
    <p style="color: #64ffda; font-size: 0.9rem; margin: 0.5rem 0 0 0;">⚡ Optimized for Cloud · Fast & Lightweight</p>
</div>
...
with st.sidebar:
    ...
    - ✅ **MCC:** ≤ {MCC_MAX:.1f}%
    - ✅ **Samples:** {N_SAMPLES} (Lightweight)
    - ✅ **Epochs:** {ADAM_EPOCHS}
    - ✅ **Network:** 128 neurons (Cloud-Optimized)
    """)
    st.info("🔬 **v29.26** — Optimized for Streamlit Cloud")
...
with col_right:
    ...
    if predict_btn:
        if abs(total - 100) > 0.1:
            ...
            with st.spinner(f"🔄 NSGA-II (pop={NSGA_POP_SIZE})..."):
...
st.markdown("---")
st.caption("🔬 **PINN v29.26** — Cloud-Optimized | Nile Valley University")
