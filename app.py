...
def load_pinn_model():
    ...
    for epoch in range(500):
        optimizer.zero_grad()
        total_loss, _, _ = model.compute_loss(X_train_t, y_train_t, 0.8, 0.2)
        total_loss.backward()
        optimizer.step()
...
with col_right:
    ...
    if predict_btn:
        ...
        with st.expander("Click to view feature importance"):
            ...
            for i in range(8):
                perturb = 0.05 * ( [85, 2, 1, 0.2, 0.5, 100, 10, 50][i] )
                test_inputs = base_inputs.copy()
                test_inputs[i] += perturb
                _, efrf_pos = predict_formulation(model, scaler, test_inputs)
                
                test_inputs[i] = base_inputs[i] - perturb
