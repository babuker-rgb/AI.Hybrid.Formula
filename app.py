...
Version: 29.16 (Light Physics: No Ryshkewitch, Data Weight 8.0)
"""
...
# ================================================================
# 0. USER-CONFIGURABLE PARAMETERS (v29.16 - LIGHT PHYSICS)
# ================================================================
...
# --- Hyperparameters (More samples, longer training) ---
N_SAMPLES = 8000            # More data
ADAM_EPOCHS = 1200          # Longer training
LBFGS_STEPS = 3
MONOTONICITY_FREQUENCY = 10

# --- NSGA-II Parameters (Unchanged) ---
...
# --- Loss Weights (LIGHT PHYSICS: Ryshkewitch = 0, Tensile Physics = 0) ---
W_DENSITY = 12.0
TENSILE_DATA_WEIGHT = 8.0         # Very high → forces fit to tensile data
TENSILE_PHYSICS_WEIGHT = 0.0       # REMOVED: no tensile physics
TENSILE_BINDER_WEIGHT = 0.0        # REMOVED: no binder physics
...
RYSK_SIGMA0_LOG = 1.2
RYSK_B = 2.0
RYSK_LOSS_WEIGHT = 0.0             # ZERO: completely eliminated
...
class MultiTaskTruePINN(nn.Module):
    ...
    def compute_loss(self, X_scaled, X_raw, y_true, epoch=0, max_epochs=ADAM_EPOCHS,
        ...
        # --- RYSZHKEWITCH-DUCKWORTH (REMOVED - weight = 0) ---
        # COMPLETELY ELIMINATED - no longer part of loss
        ...
        # --- TOTAL LOSS (Data dominates, only essential physics) ---
        total_loss = (
            w_data * data_loss +
...
def generate_full_pdf_report(api, mcc, pvpp, mgst, binder, pressure, speed, granule,
    ...
    pdf.cell(0, 6, sanitize_text("Hybrid AI Framework (PINN + NSGA-II) - v29.16"), ln=True, align="C")
    ...
    pdf.cell(0, 6, "Generated by: Hybrid AI Framework v29.16", ln=True, align="C")
...
# ================================================================
# 7. NSGA-II (UNCHANGED - includes smart seeding)
# ================================================================
...
def plot_training_curves(loss_history):
    ...
    fig.update_layout(
        title='Training Curves (v29.16 - Light Physics)',
...
# st.cache_resource.clear()   # Commented to prevent re-training each time

st.set_page_config(page_title="PINN Framework v29.16", page_icon="🧬", layout="wide")
clamp_session_state()
...
<div style="background: linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%);
    ...
    <p style="color: #ffd700; font-size: 0.85rem; margin: 0.5rem 0 0 0;">
        ⚡ v29.16 — Light Physics: No Ryshkewitch, Data Weight 8.0
    </p>
</div>
...
with st.sidebar:
    st.markdown("### 📚 Physics Constraints (v29.16)")
    st.markdown(f"""
    ...
    **Multi-Task PINN (v29.16 - Light Physics):**
    ...
    """)
    st.info(f"🔬 **v29.16** — Light Physics (Target R² ≥ 0.85)")

with st.spinner("🔄 Training Multi-Task PINN (v29.16 Light Physics)..."):
    model, scaler, y_scaler, feature_names, df, loss_history = load_pinn_model()
st.success("✅ Multi-Task True PINN (v29.16) trained successfully")
...
with col_left:
    ...
    predict_btn = st.button("🔬 Predict & Optimize (v29.16)", use_container_width=True)

# ================================================================
# RESULTS & TABS (SAME AS v29.15)
# ================================================================
with col_right:
    st.markdown("### 📈 Results")
    ...
    if predict_btn:
        if abs(total - 100) > 0.1:
            ...
            with st.spinner("🧠 Running prediction (v29.16)..."):
            ...
            with st.expander("🔬 Physics Verification (v29.16 Light Physics)"):
                try:
                    ...
                    st.caption("v29.16: Light Physics (Ryshkewitch removed)")
                except:
                    pass
    ...
    with tab1:
        ...
        if predict_btn and objectives is not None and fronts is not None and len(fronts) > 0:
            ...
            st.info("👆 Please click 'Predict & Optimize (v29.16)' with a valid formulation (total = 100%) to generate the Pareto Front.")
    ...
    with tab2:
        ...
        if predict_btn and api_norm is not None:
            ...
            st.info("👆 Please click 'Predict & Optimize (v29.16)' with a valid formulation to run Sensitivity Analysis.")

    with tab3:
        st.markdown("### 📊 Model Performance Comparison (v29.16)")
        ...
        if predict_btn and not comp_df.empty:
            ...
            st.metric("PINN R² (v29.16)", f"{pinn_r2_val:.4f}", delta="Target: ≥ 0.85")
            ...
            with col1:
                ...
                fig_r2.update_layout(
                    title='<b>R² Score Comparison (v29.16 Light Physics)</b>',
            ...
            with col2:
                ...
                fig_rmse.update_layout(
                    title='<b>RMSE Comparison (v29.16)</b>',
            ...
            if predict_btn:
                ...
                st.info("👆 Please click 'Predict & Optimize (v29.16)' with a valid formulation to see model performance comparison.")

    with tab4:
        st.markdown("### 📈 Training Curves (v29.16)")
    ...
    with tab5:
        ...
        if predict_btn and not comp_df.empty:
            ...
            if predict_btn:
                ...
                st.info("👆 Please click 'Predict & Optimize (v29.16)' with a valid formulation to generate the report.")

st.markdown("---")
st.caption(f"🔬 **Multi-Task True PINN — v29.16 (Light Physics: No Ryshkewitch, Data Weight 8.0)**")
