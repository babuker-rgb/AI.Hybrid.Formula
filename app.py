...
Version: 28.3 (Increased Density Penalty Weight)
"""
...
W_DENSITY = 5.0             # Increased from 2.0 to enforce density constraint
...
class MultiTaskTruePINN(nn.Module):
    ...
    def compute_loss(self, X_scaled, X_raw, y_true, epoch=0, max_epochs=4000,
                     ...
                     w_mcc=0.5, w_density=W_DENSITY,  # Increased to 5.0
...
def generate_full_pdf_report(api, mcc, pvpp, mgst, binder, pressure, speed, granule,
    ...
    pdf.cell(0, 6, "Generated by: Hybrid AI Framework v28.3", ln=True, align="C")
...
st.set_page_config(page_title="PINN Framework v28.3", page_icon="🧬", layout="wide")
clamp_session_state()
...
with st.sidebar:
    ...
    **Multi-Task PINN:**
    ...
    - **Density penalty weight:** {W_DENSITY:.1f} (increased)
    - **All constraints unified** ✅
    """)
    st.info("🔬 **v28.3** — Increased Density Penalty Weight")
...
st.markdown("---")
st.caption(f"🔬 **Multi-Task True PINN — v28.3 (Increased Density Penalty Weight)**")
