...
Version: 29.21 (Stable: NaN protection, log-only loss, conservative bounds)
"""
...
# ================================================================
# 0. USER-CONFIGURABLE PARAMETERS (v29.21)
# ================================================================
...
# --- Loss Weights (Log-only for stability) ---
W_DENSITY = 1.5
W_TENSILE_LOG = 5.0          # Only log loss now
W_PHYSICS_BASE = 0.5
W_PHYSICS_FINAL = 1.5
W_EFRF = 2.0
...
DENSITY_PREFERENCE_WEIGHT = 0.3
DENSITY_PREFERENCE_LIMIT = 0.95

PHYSICS_WEIGHT_INIT = 0.3

# ================================================================
# 1. SAFE IMPORTS & SESSION STATE (UNCHANGED - see full code)
# ================================================================
...
# ================================================================
# 3. PINN MODEL (v29.21 - STABLE, NaN PROTECTED)
# ================================================================
...
class MultiTaskTruePINN(nn.Module):
    def __init__(self, input_dim=13, output_dim=5):
        ...
        # Conservative bounds for tensile: 0.5 to 12 MPa
        self.sigma_min = 0.5
        self.sigma_max = 12.0
    ...
    def forward(self, X):
        ...
        # Sigmoid bounded tensile (safe gradients)
        tensile_scaled = self.sigma_min + (self.sigma_max - self.sigma_min) * torch.sigmoid(raw[:, 1:2])
        # Apply nan protection
        tensile_scaled = torch.nan_to_num(tensile_scaled, nan=1.0, posinf=12.0, neginf=0.5)
        er = torch.nn.functional.softplus(raw[:, 2:3]) + 1e-4
        er = torch.nan_to_num(er, nan=1.0, posinf=4.0, neginf=0.5)
    ...
    def compute_loss(self, X_scaled, X_raw, y_true, epoch=0, max_epochs=ADAM_EPOCHS,
        ...
        # Extract and ensure no NaN
        density_pred = torch.nan_to_num(y_pred[:, 0:1], nan=0.5, posinf=0.97, neginf=0.4)
        tensile_pred = torch.nan_to_num(y_pred[:, 1:2], nan=2.0, posinf=12.0, neginf=0.5)
        er_pred = torch.nan_to_num(y_pred[:, 2:3], nan=1.0, posinf=4.0, neginf=0.5)
        k_pred = torch.nan_to_num(y_pred[:, 3:4], nan=0.03, posinf=0.1, neginf=0.005)
        A_pred = torch.nan_to_num(y_pred[:, 4:5], nan=1.5, posinf=2.5, neginf=0.5)
        ...
        # Data loss: only log-space for tensile (stable)
        ...
        log_tensile_pred = torch.clamp(log_tensile_pred, min=-5.0, max=5.0)
        log_tensile_true = torch.clamp(log_tensile_true, min=-5.0, max=5.0)
        tensile_data_loss = nn.MSELoss()(log_tensile_pred, log_tensile_true)
        er_data_loss = nn.MSELoss()(er_pred, y_true[:, 2:3])
        data_loss = density_data_loss + w_tensile_log * tensile_data_loss + er_data_loss
        ...
        # Monotonicity (tensile vs density)
        ...
        if torch.isnan(total_loss):
            total_loss = torch.tensor(1.0, requires_grad=True, device=X_scaled.device)

        loss_dict = {
            'data_loss': data_loss.item(),
            'tensile_log_loss': tensile_data_loss.item(),
...
# ================================================================
# 5. PDF GENERATION (UNCHANGED - see previous full code)
# ================================================================
...
def generate_full_pdf_report(api, mcc, pvpp, mgst, binder, pressure, speed, granule,
    ...
    pdf.cell(0, 6, sanitize_text("Hybrid AI Framework (PINN + NSGA-II) - v29.21"), ln=True, align="C")
    ...
    pdf.cell(0, 6, "Generated by: Hybrid AI Framework v29.21", ln=True, align="C")
...
# ================================================================
# 6. NSGA-II (UNCHANGED - full code from previous versions)
# ================================================================
...
class NSGAII:
    ...
    def _smart_seed(self, n_seeds):
        ...
        while len(seeds) < n_seeds and attempts < max_attempts:
            ...
            candidate = np.array([
                ...
                np.random.uniform(0.5, 6.0),
                np.random.uniform(0.01, 1.2),
                np.random.uniform(BINDER_MIN, BINDER_MAX),
        ...
        while len(seeds) < n_seeds:
            ...
            seeds.append(self._repair(candidate))
    ...
    def _evaluate(self, population):
        ...
        return objectives, constraints, constraint_violation, tensile_strengths, population
    ...
    def _fast_non_dominated_sort(self, objectives, constraints, constraint_violation):
        ...
        if len(feasible_indices) > 0:
            ...
            n_dom = np.zeros(n)
            current_front = []
            for i_idx, i in enumerate(feasible_indices):
    ...
    def run(self):
        ...
        for gen in range(self.n_generations):
            ...
            fronts, ranks = self._fast_non_dominated_sort(objectives, constraints, constraint_violation)
            self.fronts = fronts
            ...
            for front in fronts:
                ...
                for idx, d in zip(front, dist):
                    crowding[idx] = d
            selected = self._tournament_selection(range(self.pop_size), objectives, ranks, crowding, constraint_violation)
            offspring = []
            ...
            objectives_off, constraints_off, violation_off, tensile_off, offspring = self._evaluate(offspring)
            combined_pop = np.vstack([self.population, offspring])
            combined_obj = np.vstack([self.objectives, objectives_off])
            combined_constraints = np.vstack([self.constraints, constraints_off])
            combined_violation = np.concatenate([constraint_violation, violation_off])
            combined_fronts, _ = self._fast_non_dominated_sort(combined_obj, combined_constraints, combined_violation)
            combined_crowding = np.zeros(len(combined_pop))
            for front in combined_fronts:
                dist = self._crowding_distance(combined_obj, front)
                for idx, d in zip(front, dist):
                    combined_crowding[idx] = d
            new_pop, new_obj, new_constraints, new_violation = [], [], [], []
            for front in combined_fronts:
                if len(new_pop) + len(front) <= self.pop_size:
                    for idx in front:
                        new_pop.append(combined_pop[idx])
                        new_obj.append(combined_obj[idx])
                        new_constraints.append(combined_constraints[idx])
                        new_violation.append(combined_violation[idx])
                else:
        ...
        self.constraints = constraints
        self.tensile = tensile
        self.fronts, _ = self._fast_non_dominated_sort(objectives, constraints, constraint_violation)
        return self.population, self.objectives, self.constraints, self.fronts

# ================================================================
# 7. TRAIN MODEL (v29.21 - with NaN protection)
# ================================================================

@st.cache_resource
def load_pinn_model(version="v29.21"):
    ...
    optimizer_adam = optim.Adam(model.parameters(), lr=0.0005, weight_decay=1e-4)  # Lower LR
    ...
    with torch.no_grad():
        ...
        # Remove any NaN or inf before R2
...
# ================================================================
# 8. PREDICTION & PLOTS (UNCHANGED - see v29.19 for reference)
# ================================================================
...
def plot_training_curves(loss_history):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=epochs, y=loss_history['train'], mode='lines', name='Training Loss', line=dict(color='blue', width=2)))
    if len(loss_history['val']) > 0:
        fig.add_trace(go.Scatter(x=epochs[:len(loss_history['val'])], y=loss_history['val'], mode='lines', name='Validation Loss', line=dict(color='orange', width=2, dash='dash')))
    ...
    fig.update_layout(title='Training Curves (v29.21 - Stable)', xaxis_title='Epoch', yaxis_title='Loss', yaxis_type='log', height=400, hovermode='x unified')
    return fig
...
def plot_pareto_plotly(objectives, constraints, fronts, nsga, api, efrf):
    ...
    try:
        ...
        if constraints is not None and len(constraints.shape) == 2 and constraints.shape[1] >= 2:
            g1 = constraints[front0, 0]; g2 = constraints[front0, 1]
            feasible = (g1 <= 1e-6) & (g2 <= 1e-6)
        else:
            feasible = np.ones(len(front0), dtype=bool)
        valid_mask = (pareto_api >= 85) & (pareto_api <= 95) & (pareto_efrf >= 0.0001) & (pareto_efrf <= 5.0)
        pareto_api = pareto_api[valid_mask]; pareto_efrf = pareto_efrf[valid_mask]; feasible = feasible[valid_mask]
        if len(pareto_api) == 0:
            return None
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=-objectives[:, 0], y=objectives[:, 1], mode='markers', marker=dict(size=4, color='gray', opacity=0.35), name='All Solutions'))
        fig.add_trace(go.Scatter(x=pareto_api, y=pareto_efrf, mode='lines+markers', marker=dict(size=8, color='red'), line=dict(color='red', width=2), name='Pareto Front'))
        feas_x = pareto_api[feasible]; feas_y = pareto_efrf[feasible]
        if len(feas_x) > 0:
            fig.add_trace(go.Scatter(x=feas_x, y=feas_y, mode='markers', marker=dict(size=10, color='green', symbol='diamond'), name='Feasible Solutions'))
        safe_mask = (feas_x >= DENSITY_LOWER_CONSTRAINT) & (feas_x <= DENSITY_UPPER_CONSTRAINT) & (feas_y < 0.36)
        if np.any(safe_mask):
            fig.add_trace(go.Scatter(x=feas_x[safe_mask], y=feas_y[safe_mask], mode='markers', marker=dict(size=12, color='gold', symbol='star'), name='Preferred Safe Zone'))
        if api and efrf and 85 <= api <= 95 and 0.0001 <= efrf <= 2:
            fig.add_trace(go.Scatter(x=[api], y=[efrf], mode='markers+text', marker=dict(size=14, color='blue', symbol='star'), text=[f"Your Formulation<br>API: {api:.1f}%<br>EFRF: {efrf:.4f}"], name='Selected Formulation'))
        ...
        fig.update_layout(title='Pareto Front: API vs EFRF (Gold = Safe Zone)', xaxis_title='API (%)', yaxis_title='EFRF', template='plotly_white', height=500, legend=dict(orientation='h', yanchor='bottom', y=1.02, xanchor='right', x=1))
        return fig
    except Exception as e:
...
def plot_sensitivity_plotly(inputs, model, scaler, y_scaler):
    ...
    try:
        ...
        fig.add_trace(go.Bar(y=sorted_names, x=sorted_values, orientation='h', marker=dict(color=colors), text=[f"{v:.4f}" for v in sorted_values], textposition='outside'))
        fig.add_vline(x=np.mean(sensitivities), line_dash='dash', line_color='red', annotation_text=f'Avg: {np.mean(sensitivities):.4f}')
        fig.update_layout(title='Sensitivity Analysis (EFRF)', xaxis_title='Sensitivity (ΔEFRF)', yaxis_title='Parameters', height=450)
        return fig
    except Exception:
        return None
...
st.set_page_config(page_title="PINN Framework v29.21", page_icon="🧬", layout="wide")
clamp_session_state()
...
<div style="background: linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%);
    ...
    <p style="color: #ffd700; font-size: 0.85rem; margin: 0.5rem 0 0 0;">
        ⚡ v29.21 — Stable: NaN protection, log-only loss, conservative bounds
    </p>
</div>
...
with st.sidebar:
    st.markdown("### 📚 Physics Constraints (v29.21)")
    st.markdown(f"""
    ...
    - ✅ **Tensile Physics:** Sigmoid Bounding (0.5–12 MPa) + log-only loss
    - ✅ **Dynamic Scheduling:** Sigmoid annealing
    - ✅ **MCC:** ≤ {MCC_MAX:.1f}%
    - ✅ **Density:** {D_MIN:.2f} ≤ D ≤ {D_MAX:.2f}

    **Multi-Task PINN (v29.21):**
    - 5 outputs (D, σt, ER, k, A)
    - Adam ({ADAM_EPOCHS}) with lr=0.0005, wd=1e-4
    - Network: 384→384→192 with SiLU, NaN protection
    - Loss: Log-space Huber for tensile, no real-space
    - Gradient Clipping: max_norm = 1.0
    - NSGA-II: pop={NSGA_POP_SIZE}, gen={NSGA_GENERATIONS}
    """)
    st.info(f"🔬 **v29.21** — Stable Release (NaN-safe)")

with st.spinner("🔄 Training Multi-Task PINN (v29.21)..."):
    model, scaler, y_scaler, feature_names, df, loss_history = load_pinn_model(version="v29.21")
st.success("✅ Multi-Task True PINN (v29.21) trained successfully")
...
with col_left:
    ...
    predict_btn = st.button("🔬 Predict & Optimize (v29.21)", use_container_width=True)

# ================================================================
# RESULTS & TABS (same as v29.20)
# ================================================================
with col_right:
    st.markdown("### 📈 Results")
    objectives = None
    ...
    if predict_btn:
        if abs(total - 100) > 0.1:
            ...
            with st.spinner("🧠 Running prediction (v29.21)..."):
            ...
            with st.expander("🔬 Physics Verification (v29.21)"):
                try:
                    ...
                    st.caption("v29.21: Stable release (NaN-safe)")
                except:
                    pass
    ...
    with tab1:
        ...
        if predict_btn and objectives is not None and fronts is not None and len(fronts) > 0:
            ...
            st.info("👆 Please click 'Predict & Optimize (v29.21)' with a valid formulation (total = 100%) to generate the Pareto Front.")
    ...
    with tab2:
        ...
        if predict_btn and api_norm is not None:
            ...
            st.info("👆 Please click 'Predict & Optimize (v29.21)' with a valid formulation to run Sensitivity Analysis.")

    with tab3:
        st.markdown("### 📊 Model Performance Comparison (v29.21)")
        ...
        if predict_btn and not comp_df.empty:
            ...
            st.metric("PINN R² (v29.21)", f"{pinn_r2_val:.4f}", delta="Target: ≥ 0.85")
            ...
            with col1:
                ...
                fig_r2.update_layout(title='<b>R² Score Comparison (v29.21)</b>', yaxis_title='R² Score', range=[0, 1.05], height=350, showlegend=False)
            ...
            with col2:
                ...
                fig_rmse.update_layout(title='<b>RMSE Comparison (v29.21)</b>', yaxis_title='RMSE (MPa)', height=350, showlegend=False)
            ...
            if predict_btn:
                ...
                st.info("👆 Please click 'Predict & Optimize (v29.21)' with a valid formulation to see model performance comparison.")

    with tab4:
        st.markdown("### 📈 Training Curves (v29.21)")
    ...
    with tab5:
        ...
        if predict_btn and not comp_df.empty:
            ...
            if predict_btn:
                ...
                st.info("👆 Please click 'Predict & Optimize (v29.21)' with a valid formulation to generate the report.")

st.markdown("---")
st.caption(f"🔬 **Multi-Task True PINN — v29.21 (Stable: NaN protection, log-only loss)**")
